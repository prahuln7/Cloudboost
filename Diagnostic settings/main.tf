...
locals {
  ...
  defaultname        = local.namevalidation == "true" ? "${local.namelist[0]}-Diagnostic setting-${var.applicationname}-${var.environment}" : ""
...
resource "azurerm_monitor_diagnostic_setting" "main" {
  ...
  dynamic "metric" {
    ...
    content {
      ...
      retention_policy {
        enabled = var.retention_policy_metrics_enable
      }
    }
  }
}